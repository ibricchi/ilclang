...
def main():
    ...
    parser_random_clang_pgo = subparsers.add_parser('random-clang-pgo', help='randomly flip decision to inline')
    parser_random_clang_pgo.add_argument('pgo_file', type=str, help='path to pgo file')
    parser_random_clang_pgo.add_argument('flip_rate', type=float, help='rate of flipping decision')
    ...
    if args.mode == 'no-inline':
        ...
        result = run_no_inlining(compiler, args.log, args.decision, args.final_callgraph, args.cli)
    elif args.mode == 'random-inline':
        from controllers.random import run_random_inlining
        result = run_random_inlining(compiler, args.log, args.decision, args.final_callgraph, args.cli, args.flip_rate, args.seed)
    elif args.mode == 'random-clang-pgo':
        from controllers.random_clang_pgo import run_random_clang_pgo
        result = run_random_clang_pgo(compiler, args.log, args.decision, args.final_callgraph, args.cli, args.pgo_file, args.flip_rate, args.seed)
    else:
